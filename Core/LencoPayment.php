...
class LencoPayment extends HttpVerbs{
    protected $baseUrl;
    protected $secKey;
    protected $pubKey;
    ...
    function __construct()
    {
        ...
        if(!Session::get('bearer_token_lenco')){
            /*make an authencation request to the endpoint to fetch the bearer token */
            $response = $this->authenticate($endpoint, $keys);
            /**token */
            Session::set('bearer_token_lenco',$response);
        }

        $this->bearerToken = trim(Session::get('bearer_token_lenco'));
    }
